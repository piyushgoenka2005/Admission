...
async function runCoverGeneration(
  ...
  throw lastError instanceof Error ? lastError : new Error('Failed to execute generate_cover.py');
}

async function handleCover(request: Request) {
  let tempOutputDir = '';
  ...
  try {
    ...
    const scriptPath = path.join(process.cwd(), 'scripts', 'generate_cover.py');
    tempOutputDir = fs.mkdtempSync(path.join(os.tmpdir(), 'websa-cover-'));
    const result = await runCoverGeneration(scriptPath, studentPayload, tempOutputDir, {
    ...
    if (mode === 'print') {
      return NextResponse.json(
        {
          success: true,
          message: 'Application Cover sent to printer/opened in Word on server.',
    ...
    if (!filePath || !fs.existsSync(filePath)) {
      return NextResponse.json(
        { error: 'Application Cover generated but output file was not found.' },
        { status: 500 }
      );
    }
    ...
    const message = error instanceof Error ? error.message : 'Failed to generate application cover';
