...
TEMPLATE_FILE = "SampleGALetter.docx"
...
def build_replacements(s):
    ...
    month      = s.get("month", "") or s.get("sl. no.", "").split("/")[0] if False else s.get("month", "")
...
def generate_letter(template_path, student, output_folder):
    ...
    for para in doc.paragraphs:
        ...
        if "{{Name_College}}" in merged and "Dean/HOD" in merged:
            for run in para.runs:
                run.text = ""
            if para.runs:
...
def main():
    ...
    if args.student_json:
        ...
        student = {
            ...
            "college name": str(parsed.get("college", "")).strip(),
            "district": str(parsed.get("district", "")).strip(),
            "guide name": str(parsed.get("guide_name", "")).strip(),
            "guide area": str(parsed.get("guide_area", "")).strip(),
            "course": str(parsed.get("course", "")).strip(),
            "start date": str(parsed.get("start_date", "")).strip(),
            "end date": str(parsed.get("end_date", "")).strip(),
            "allotment date": str(parsed.get("allotment_date", "")).strip(),
            "email": str(parsed.get("email", "")).strip(),
            "month": str(parsed.get("month", "")).strip(),
            "sl. no.": str(parsed.get("sl_no", "")).strip(),
            "college dean hod": str(parsed.get("college_dean_hod", "")).strip(),
            "guide reporting officer": str(parsed.get("guide_reporting_officer", "")).strip(),
